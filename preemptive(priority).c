#include<stdio.h>
void sort(int n,int P[],int AT[],int BT[],int PR[],int BBT[])
{
	int i,j;
	for(i=0;i<n;i++)
	{
...
int Highest_PR(int n,int t,int AT[],int BT[],int PR[],int V[])
{
...
int main()
{
	...
	scanf("%d",&n);
	int P[n],AT[n],BT[n],PR[n],BBT[n],CT[n],TAT[n],WT[n],V[100];
	...
	sort(n,P,AT,BT,PR,BBT);
	printf("\n After Sorting is : \n");
	printf("\n\tPID\tPR\tAT\tBT\tBBT");
	printf("\n");
	for(i=0;i<n;i++)
	{
		printf("\t%d\t%d\t%d\t%d\t%d",P[i],PR[i],AT[i],BT[i],BBT[i]);
		printf("\n");
	}
